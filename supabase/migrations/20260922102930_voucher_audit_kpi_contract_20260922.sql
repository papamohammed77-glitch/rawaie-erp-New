-- RAWAEA ERP — VOUCHER_AUDIT lifecycle KPI contract
-- Additive only: no new tables/columns/Edge Functions; existing response keys preserved.
CREATE OR REPLACE FUNCTION public.inventory_control(p_operation text, p_payload jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  IF v_op='VOUCHER_AUDIT' THEN
    ...
    RETURN jsonb_build_object(
      ...
      'kpi',jsonb_build_object(
        'start_at',v_voucher.created_at,
        'sent_at',v_voucher.sent_date,
        'received_at',v_voucher.received_date,
        'completed_at',v_voucher.completed_at,
        'end_at',coalesce(v_voucher.completed_at,v_voucher.received_date),
        'created_to_sent_seconds',case when v_voucher.created_at is not null and v_voucher.sent_date is not null then extract(epoch from (v_voucher.sent_date-v_voucher.created_at)) else null end,
        'sent_to_received_seconds',case when v_voucher.sent_date is not null and v_voucher.received_date is not null then extract(epoch from (v_voucher.received_date-v_voucher.sent_date)) else null end,
        'received_to_completed_seconds',case when v_voucher.received_date is not null and v_voucher.completed_at is not null then extract(epoch from (v_voucher.completed_at-v_voucher.received_date)) else null end,
        'created_to_completed_seconds',case when v_voucher.created_at is not null and v_voucher.completed_at is not null then extract(epoch from (v_voucher.completed_at-v_voucher.created_at)) else null end,
        'current_age_seconds',case when coalesce(v_voucher.status,'') not in ('Completed','Cancelled') and v_voucher.created_at is not null then greatest(0,extract(epoch from (now()-v_voucher.created_at))) else null end
      )
    );
  END IF;
