-- Production closure snapshot: voucher company-scope, audit actor and operation identity
BEGIN;
CREATE OR REPLACE FUNCTION public.complete_manual_stock_voucher_atomic_core_20260828(p_company_id uuid, p_voucher_code text, p_user_email text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.cancel_manual_stock_voucher_atomic_core_20260828(p_company_id uuid, p_voucher_code text, p_user_email text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.post_manual_stock_voucher_atomic(p_company_id uuid, p_voucher_code text, p_operation text, p_user_email text, p_effects jsonb, p_operation_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
