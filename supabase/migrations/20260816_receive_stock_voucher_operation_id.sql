-- Corrective migration: replace quantity-derived RECEIVE identity with a request-level operation_id.
...
BEGIN
  ...
  IF p_operation='RECEIVE' AND NULLIF(btrim(p_operation_id),'') IS NULL THEN RAISE EXCEPTION 'operation_id مطلوب لاستلام الإذن'; END IF;
  ...
  FOR e IN SELECT * FROM jsonb_to_recordset(p_effects) AS x(direction text,branch_id uuid,item_id uuid,item_code text,qty numeric) LOOP
    ...
    SELECT d.qty INTO v_detail_qty FROM public.stock_voucher_details d WHERE d.voucher_id=v.id AND d.item_id=e.item_id AND d.item_code=e.item_code FOR UPDATE;
    ...
    IF p_operation='SEND' THEN
      ...
      movement:=CASE v.type WHEN 'Transfer' THEN 'TransferOut' WHEN 'DirectSale' THEN 'DirectSale' WHEN 'SupplierReturn' THEN 'SupplierReturn' END; src:=v.from_id; tgt:=NULL;
    ELSE
      ...
      PERFORM 1 FROM public.stock_voucher_details d WHERE d.voucher_id=v.id AND d.item_id=e.item_id AND d.item_code=e.item_code AND e.qty <= (d.qty-coalesce(d.received_qty,0));
      IF NOT FOUND THEN
        idem:='ManualVoucher:RECEIVE:'||p_company_id::text||':'||v.id::text||':'||NULLIF(btrim(p_operation_id),'')||':'||e.item_id::text;
        IF NOT EXISTS(SELECT 1 FROM public.inventory_log il WHERE il.company_id=p_company_id AND il.idempotency_key=idem AND il.item_id=e.item_id) THEN RAISE EXCEPTION 'الكمية المستلمة أكبر من المتبقي'; END IF;
      END IF;
      movement:=CASE v.type WHEN 'Transfer' THEN 'TransferIn' WHEN 'DirectReturn' THEN 'DirectReturn' END; src:=NULL; tgt:=v.to_id;
    END IF;
    idem:=CASE WHEN p_operation='RECEIVE' THEN 'ManualVoucher:RECEIVE:'||p_company_id::text||':'||v.id::text||':'||NULLIF(btrim(p_operation_id),'')||':'||e.item_id::text ELSE 'ManualVoucher:SEND:'||p_company_id::text||':'||v.id::text||':'||e.item_id::text||':'||e.qty::text END;
  ...
  END LOOP;
  IF p_operation='SEND' THEN
    UPDATE public.stock_vouchers SET status='Sent',sent_date=now() WHERE id=v.id AND company_id=p_company_id AND status='Draft';
  ELSE
    SELECT count(*) INTO remain FROM public.stock_voucher_details sd WHERE sd.voucher_id=v.id AND coalesce(sd.received_qty,0)<sd.qty;
    UPDATE public.stock_vouchers SET status=CASE WHEN remain=0 THEN 'Received' ELSE 'Sent' END,received_date=CASE WHEN remain=0 THEN now() ELSE received_date END WHERE id=v.id AND company_id=p_company_id AND status='Sent';
  END IF;
  IF NOT FOUND THEN RAISE EXCEPTION 'فشل انتقال حالة الإذن'; END IF;
  RETURN jsonb_build_object('success',true,'voucher_code',p_voucher_code,'operation',p_operation,'operation_id',p_operation_id,'status',(SELECT status FROM public.stock_vouchers WHERE id=v.id));
END;
$function$;
