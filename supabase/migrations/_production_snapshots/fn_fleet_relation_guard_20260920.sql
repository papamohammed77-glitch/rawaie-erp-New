...
BEGIN
  IF TG_TABLE_NAME='fleet_drivers' THEN
    IF NEW.user_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.users u WHERE u.id=NEW.user_id AND u.company_id=NEW.company_id
    ) THEN RAISE EXCEPTION 'fleet driver user/company mismatch'; END IF;

  ELSIF TG_TABLE_NAME='fleet_driver_documents' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.fleet_drivers d WHERE d.id=NEW.fleet_driver_id AND d.company_id=NEW.company_id
    ) THEN RAISE EXCEPTION 'driver document/company mismatch'; END IF;

  ELSIF TG_TABLE_NAME='fleet_vehicle_assignments' THEN
    IF NOT EXISTS (SELECT 1 FROM public.vehicles v WHERE v.id=NEW.vehicle_id AND v.company_id=NEW.company_id) THEN
      RAISE EXCEPTION 'assignment vehicle/company mismatch';
    END IF;
    ...
    IF NEW.driver_user_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.users u WHERE u.id=NEW.driver_user_id AND u.company_id=NEW.company_id
    ) THEN RAISE EXCEPTION 'assignment user/company mismatch'; END IF;

  ELSIF TG_TABLE_NAME='fleet_vehicle_contracts' THEN
    IF NOT EXISTS (SELECT 1 FROM public.vehicles v WHERE v.id=NEW.vehicle_id AND v.company_id=NEW.company_id) THEN
      RAISE EXCEPTION 'contract vehicle/company mismatch';
    END IF;
    IF NEW.supplier_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.suppliers s WHERE s.id=NEW.supplier_id AND s.company_id=NEW.company_id
    ) THEN RAISE EXCEPTION 'contract supplier/company mismatch'; END IF;
    IF NEW.responsible_user_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.users u WHERE u.id=NEW.responsible_user_id AND u.company_id=NEW.company_id
    ) THEN RAISE EXCEPTION 'contract responsible user/company mismatch'; END IF;

  ELSIF TG_TABLE_NAME='fleet_fuel_transactions' THEN
    IF NOT EXISTS (SELECT 1 FROM public.vehicles v WHERE v.id=NEW.vehicle_id AND v.company_id=NEW.company_id) THEN
      RAISE EXCEPTION 'fuel vehicle/company mismatch';
    END IF;
    IF NEW.fleet_driver_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.fleet_drivers d WHERE d.id=NEW.fleet_driver_id AND d.company_id=NEW.company_id
    ...
    IF NEW.runsheet_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.runsheets r WHERE r.id=NEW.runsheet_id AND r.company_id=NEW.company_id AND r.vehicle_id=NEW.vehicle_id
    ) THEN RAISE EXCEPTION 'fuel runsheet/vehicle/company mismatch'; END IF;

  ELSIF TG_TABLE_NAME='fleet_maintenance_plans' THEN
    IF NOT EXISTS (SELECT 1 FROM public.vehicles v WHERE v.id=NEW.vehicle_id AND v.company_id=NEW.company_id) THEN
      RAISE EXCEPTION 'maintenance plan vehicle/company mismatch';
    END IF;
    IF NEW.next_service_odometer_km IS NOT NULL AND NEW.next_service_odometer_km < 0 THEN
      RAISE EXCEPTION 'next service odometer cannot be negative';
    END IF;
  ...
  ELSIF TG_TABLE_NAME='fleet_incidents' THEN
    ...
    IF NEW.runsheet_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.runsheets r WHERE r.id=NEW.runsheet_id AND r.company_id=NEW.company_id
        AND (NEW.vehicle_id IS NULL OR r.vehicle_id=NEW.vehicle_id)
  ...
  ELSIF TG_TABLE_NAME='fleet_driver_performance_events' THEN
    ...
    IF NEW.incident_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.fleet_incidents i WHERE i.id=NEW.incident_id AND i.company_id=NEW.company_id
  ...
  ELSIF TG_TABLE_NAME='vehicle_tracking' THEN
    IF NEW.vehicle_id IS NULL OR NOT EXISTS (
      SELECT 1 FROM public.vehicles v WHERE v.id=NEW.vehicle_id AND v.company_id=NEW.company_id
    ) THEN RAISE EXCEPTION 'vehicle tracking vehicle/company mismatch'; END IF;
    IF NEW.driver_id IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM public.users u WHERE u.id=NEW.driver_id AND u.company_id=NEW.company_id
