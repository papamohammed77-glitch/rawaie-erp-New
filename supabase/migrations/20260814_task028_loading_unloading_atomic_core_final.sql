...
BEGIN
    ...
    IF p_idempotency_key IS NOT NULL THEN
        ...
        LIMIT 1;
        IF FOUND THEN
            IF v_existing_log.movement_type <> p_movement_type OR v_existing_log.qty <> p_qty THEN
    ...
    END IF;
    IF v_source_required AND v_target_required AND p_source_branch_id = p_target_branch_id THEN
    ...
    IF p_target_branch_id IS NOT NULL THEN
        ...
        END IF;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM public.items WHERE id = p_item_id AND company_id = p_company_id) THEN
    ...
    IF v_source_required THEN
        ...
        SET qty = v_source_stock.qty - p_qty,
            allocated_qty = CASE WHEN p_movement_type = 'Loading' THEN v_source_stock.allocated_qty - p_qty ELSE v_source_stock.allocated_qty END,
            updated_at = now()
    ...
    IF v_target_required THEN
        ...
        SET qty = v_target_stock.qty + p_qty,
            allocated_qty = CASE WHEN p_movement_type = 'Unloading' THEN v_target_stock.allocated_qty + p_qty ELSE v_target_stock.allocated_qty END,
            updated_at = now()
...
CREATE OR REPLACE FUNCTION public.post_stock_movement(
    ...
    p_item_id uuid,p_qty numeric,p_voucher_id text,p_reference text,p_user_email text
) RETURNS jsonb
...
BEGIN
    ...
    RETURN public.post_stock_movement(
        p_company_id,p_movement_type,p_source_branch_id,p_target_branch_id,p_item_id,p_qty,
        p_voucher_id,p_reference,p_user_email,NULL
    );
END;
$$;

CREATE OR REPLACE FUNCTION public.complete_runsheet_loading(
    p_company_id uuid,p_runsheet_id uuid,p_items jsonb,p_user_email text
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE
...
BEGIN
    ...
    SELECT md5(COALESCE(string_agg(x.item_code || ':' || x.loaded_qty::text,'|' ORDER BY x.item_code),''))
    INTO v_operation_hash
    FROM jsonb_to_recordset(p_items) x(item_code text,loaded_qty numeric);
    v_idempotency_key_base := 'TASK-028|Loading|'||v_rs.id::text||'|'||v_rs.loader_start::text||'|'||v_operation_hash;
    ...
    LOOP
        ...
        SELECT COALESCE(SUM(GREATEST(COALESCE(od.qty_picked,0)-COALESCE(od.qty_loaded,0),0)),0) INTO v_capacity
        ...
        PERFORM public.post_stock_movement(
            p_company_id,'Loading',v_main,v_van,v_item_id,v_requested,v_rs.runsheet_code,
            v_idempotency_key_base||'|'||v_item_id::text,p_user_email,
            v_idempotency_key_base||'|'||v_item_id::text
        );

        v_remaining:=v_requested;
        FOR v_od IN
            SELECT od.id,COALESCE(od.qty_picked,0) qty_picked,COALESCE(od.qty_loaded,0) qty_loaded
            FROM public.order_details od JOIN public.orders o ON o.id=od.order_id
            WHERE o.company_id=p_company_id AND o.runsheet_id=p_runsheet_id AND od.item_code=v_item_code
        ...
        LOOP
            ...
            DECLARE v_delta numeric; BEGIN
                ...
                IF v_delta>0 THEN
                    UPDATE public.order_details SET qty_loaded=COALESCE(qty_loaded,0)+v_delta,updated_at=now(),
                        reason_loading=CASE WHEN COALESCE(qty_picked,0)>COALESCE(qty_loaded,0)+v_delta THEN 'Partial Loading' ELSE reason_loading END
                    WHERE id=v_od.id;
        ...
        FOR v_od IN
            SELECT od.id order_detail_id,od.order_id,od.qty ordered_qty,od.qty_loaded,od.item_id,od.item_code
            FROM public.order_details od JOIN public.orders o ON o.id=od.order_id
...
CREATE OR REPLACE FUNCTION public.complete_runsheet_unloading(
    p_company_id uuid,p_runsheet_code text,p_user_email text
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE
...
BEGIN
    SELECT * INTO v_rs FROM public.runsheets WHERE company_id=p_company_id AND runsheet_code=p_runsheet_code FOR UPDATE;
    IF NOT FOUND THEN RAISE EXCEPTION 'runsheet not found'; END IF;
    ...
    SELECT md5(COALESCE(string_agg(item_id::text || ':' || qty_loaded::text,'|' ORDER BY item_id),''))
    INTO v_operation_hash
    ...
    FOR v_detail IN SELECT item_id,item_code,qty_loaded FROM public.run_sheet_details WHERE runsheet_id=v_rs.id AND COALESCE(qty_loaded,0)>0 ORDER BY item_id LOOP
        PERFORM public.post_stock_movement(
            p_company_id,'Unloading',v_van,v_main,v_detail.item_id,v_detail.qty_loaded,v_rs.runsheet_code,
            v_idempotency_key_base||'|'||v_detail.item_id::text,p_user_email,
    ...
    FROM public.orders o WHERE od.order_id=o.id AND o.company_id=p_company_id AND o.runsheet_id=v_rs.id AND COALESCE(od.qty_loaded,0)>0;
    UPDATE public.fulfillment_backorders SET status='Cancelled',updated_at=now() WHERE runsheet_id=v_rs.id AND status='Pending';
    UPDATE public.runsheets SET status='Picked',loader_end=NULL,updated_at=now() WHERE id=v_rs.id AND status='Loaded';
