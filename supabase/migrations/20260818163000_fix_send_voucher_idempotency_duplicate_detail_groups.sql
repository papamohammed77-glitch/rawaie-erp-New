...
BEGIN
  ...
  IF expected_count>0 AND existing_count=expected_count THEN
    RETURN jsonb_build_object('success',true,'duplicate',true,'voucher_id',v_voucher.id,'voucher_code',p_voucher_code,'status',v_voucher.status,'movement_count',existing_count);
  END IF;

  IF v_voucher.status<>'Draft' THEN RAISE EXCEPTION 'Voucher is not Draft'; END IF;
  IF v_voucher.type NOT IN('Transfer','DirectSale','SupplierReturn') THEN RAISE EXCEPTION 'Unsupported send movement type: %',v_voucher.type; END IF;
  ...
  IF NOT EXISTS(SELECT 1 FROM public.branches b WHERE b.id=v_source AND b.company_id=p_company_id) THEN
  ...
  LOOP
    ...
    IF NOT EXISTS(
      ...
      WHERE i.id=d.item_id AND i.item_code=d.item_code
    ) THEN
    ...
    SELECT public.post_stock_movement(
      p_company_id,m,v_source,NULL,d.item_id,d.qty,p_voucher_code,p_voucher_code,p_user_email,key
    ) INTO r;
  ...
  RETURN jsonb_build_object('success',true,'voucher_id',v_voucher.id,'voucher_code',p_voucher_code,'status','Sent','movement_count',move_count);
END;
$function$;
