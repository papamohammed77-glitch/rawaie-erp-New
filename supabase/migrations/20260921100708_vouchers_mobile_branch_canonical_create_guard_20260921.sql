-- RAWAEA ERP — canonical vehicle mobile-branch guard for manual voucher creation
-- Applied to Production as migration:
-- 20260921100708_vouchers_mobile_branch_canonical_create_guard_20260921
-- Purpose: keep the existing manual-voucher CREATE contract while accepting the
-- canonical vehicles.mobile_branch_id identity and preserving the legacy
-- VAN-<vehicle_code> fallback.
-- No new Edge Function. No Physical Stock writer change.

CREATE OR REPLACE FUNCTION public.create_manual_stock_voucher_atomic_core_12_20260828(
  p_company_id uuid,
  p_type text,
  p_reference text,
  p_from_type text,
...
BEGIN
  ...
  IF p_type='DirectSale' THEN
    IF p_from_type<>'Branch'
       OR p_from_id IS NULL
       OR p_to_type<>'Vehicle'
       OR p_to_id IS NULL THEN
      RAISE EXCEPTION 'الصرف المباشر يتطلب فرع مصدر ومركبة وجهة';
    END IF;
    ...
    ELSIF coalesce(v_actor.active_warehouse_role,'')='أذونات' THEN
      IF p_rep_id IS NULL THEN
        RAISE EXCEPTION 'مندوب البيع المباشر مطلوب للصرف المباشر';
      END IF;
    ELSE
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'المركبة الوجهة لا تتبع مندوب البيع المباشر المحدد أو ليست مهيأة كمخزن متنقل';
    END IF;
    ...
    ELSE
      v_allowed:=
        NULLIF(btrim(v_rep.allowed_branch_ids::text),'') IS NULL
