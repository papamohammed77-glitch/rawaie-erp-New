...
def main():
 ...
 for attempt in range(1,8):
  try:
   ...
   TARGET.write_text(edited,encoding='utf-8');static=static_gate();browser_gate();after=sha256(TARGET);update_state(before,after,static)
   run(['git','add','Current/PWA/New-main','CURRENT_STATE.md']); staged=run(['git','diff','--cached','--name-only']).stdout.splitlines()
   if set(staged)!={'Current/PWA/New-main','CURRENT_STATE.md'}:raise RuntimeError('P163_FAIL: staged paths unexpected')
   ...
   last=r.stdout[-4000:] if r.stdout else 'push race';print('P163_PUSH_RACE',attempt,last)
