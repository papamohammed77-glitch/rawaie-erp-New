...
def normalize_fragment(raw, idx):
    ...
    if idx == 2:
        ...
        supabase.from('categories').select('id, category_name').eq('company_id',_rwCompanyId()).order('category_name').then(function(res){
            ...
            var popup=Swal.getPopup();if(!popup)return;
            var editButtons=popup.querySelectorAll('[data-rw-category-edit]');for(var e=0;e<editButtons.length;e++)(function(btn){btn.addEventListener('click',function(){RW_Items._editCategory(btn.getAttribute('data-rw-category-edit'),btn.getAttribute('data-rw-category-name')||'');});})(editButtons[e]);
            var addButton=popup.querySelector('[data-rw-category-add]');if(addButton)addButton.addEventListener('click',function(){RW_Items._addCategory();});
        });
    }
'''
...
def cumulative_syntax_gate(parts):
    # MAIN1 is intentionally open; the first independently meaningful program is MAIN1+MAIN2.
    js=extract_main1_application_js(parts[0])+'\n\n'+parts[1]
    reports=[]
    for idx in range(2,len(parts)+1):
        probe=Path(tempfile.gettempdir())/f'rawaea-cumulative-{idx}.js'; probe.write_text(js,encoding='utf-8')
...
def main():
    ...
    phase_report=validate_fragments(parts); cumulative=cumulative_syntax_gate(parts)
    ...
    print({'status':'NEW_MAIN_GOLD_DIAMOND_READY','target':str(MAIN),'sha256':hashlib.sha256(candidate.encode()).hexdigest(),'bytes':len(candidate.encode()),'gates':gates,'phase_report':phase_report,'cumulative_syntax':cumulative})
