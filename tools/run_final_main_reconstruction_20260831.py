...
def delimiter_gate(js):
    pairs={')':'(',']':'[','}':'{'}; stack=[]; state='code'; quote=None; line=1; col=0; i=0
    while i<len(js):
        ch=js[i]; col+=1
        if ch=='\n': line+=1; col=0
        if state=='line':
        ...
        if ch in '([{': stack.append((ch,line,col))
        elif ch in pairs:
            if not stack or stack[-1][0]!=pairs[ch]:
                raise RuntimeError('DELIMITER_MISMATCH at %d:%d got %s top=%r'%(line,col,ch,stack[-8:]))
            stack.pop()
        i+=1
    if stack or state!='code':
        raise RuntimeError('DELIMITER_EOF stack=%r state=%s last_line=%d'%(stack[-20:],state,line))
    return {'status':'BALANCED','depth':0,'lines':line}
...
def validate(s):
    ...
    app=_app_js(s);diag=delimiter_gate(app);path=Path(tempfile.gettempdir())/'rawaea-new-main.js';path.write_text(app,encoding='utf-8');r=subprocess.run(['node','--check',str(path)],capture_output=True,text=True)
