...
def normalize_main1(raw):
    ...
    app_open=opens[-1]; prefix=raw[:app_open.start()]; body=raw[app_open.start():]; body=P1_FORENSIC_MARKER.sub('',body); body=HTML_COMMENT_TAIL.sub('',body); return prefix+body.rstrip()
...
def normalize_fragment(raw,idx):
    ...
    if idx==2:
        ...
        supabase.from('categories').select('id, category_name').eq('company_id',_rwCompanyId()).order('category_name').then(function(res){
            var categories=res.data||[];var html='<div class="text-right" dir="rtl"><h3 class="font-bold text-lg mb-3">🗂️ إدارة التصنيفات</h3>';
...
def cumulative_syntax_gate(parts):
    # Main1 intentionally remains open; parse only after Main2 closes the script boundary.
    js=extract_main1_application_js(parts[0])+'\n\n'+parts[1]; reports=[]
    for idx in range(2,len(parts)+1):
        probe=Path(tempfile.gettempdir())/f'rawaea-cumulative-{idx}.js'; probe.write_text(js+'\n',encoding='utf-8'); r=subprocess.run(['node','--check',str(probe)],capture_output=True,text=True); reports.append({'through':idx,'bytes':len(js.encode('utf-8')),'status':'PASS' if r.returncode==0 else 'FAIL'})
...
def validate(s):
    ...
    path=Path(tempfile.gettempdir())/'rawaea-new-main.js';path.write_text(_app_js(s),encoding='utf-8');r=subprocess.run(['node','--check',str(path)],capture_output=True,text=True)
