...
def normalize_fragment(raw, idx):
    ...
    if idx == 2:
        ...
        if start < 0 or end < 0 or end <= start:
            raise RuntimeError('MAIN2_CATEGORY_FUNCTION_BOUNDARY_MISSING')
        ...
        supabase.from('categories').select('id, category_name').eq('company_id',_rwCompanyId()).order('category_name').then(function(res){
            ...
            if(categories.length===0){
                ...
                for(var i=0;i<categories.length;i++){
                    ...
                    html+='<button type="button" data-rw-category-edit="'+_esc(String(catId))+'" data-rw-category-name="'+_esc(catName)+'" class="w-full flex justify-between items-center p-2 bg-gray-50 rounded-lg cursor-pointer hover:bg-indigo-50 text-right"><span class="font-bold text-sm">'+_esc(catName)+'</span></button>';
                }
                html+='</div>';
            }
            ...
            Swal.fire({title:'',html:html,width:'500px',showCloseButton:true,showConfirmButton:false,customClass:{popup:'!rounded-3xl'}});
            var popup=Swal.getPopup();
            if(!popup)return;
            var editButtons=popup.querySelectorAll('[data-rw-category-edit]');
            for(var e=0;e<editButtons.length;e++){
                (function(btn){btn.addEventListener('click',function(){RW_Items._editCategory(btn.getAttribute('data-rw-category-edit'),btn.getAttribute('data-rw-category-name')||'');});})(editButtons[e]);
            }
            var addButton=popup.querySelector('[data-rw-category-add]');
            if(addButton)addButton.addEventListener('click',function(){RW_Items._addCategory();});
        });
    }
'''
...
def cumulative_syntax_gate(parts):
    js = extract_main1_application_js(parts[0])
    reports=[]
    for idx in range(1,len(parts)+1):
        if idx > 1: js += '\n\n' + parts[idx-1]
        probe=Path(tempfile.gettempdir())/f'rawaea-cumulative-{idx}.js'
        probe.write_text(js,encoding='utf-8')
        r=subprocess.run(['node','--check',str(probe)],capture_output=True,text=True)
        reports.append({'through':idx,'bytes':len(js.encode('utf-8'))})
        if r.returncode: raise RuntimeError('CUMULATIVE_JS_FAIL:main%d\n%s' % (idx,r.stderr))
    return reports
...
def main():
    ...
    cumulative=cumulative_syntax_gate(parts)
    candidate=parts[0]+'\n\n'+'\n\n'.join(parts[1:])+'\n\n</script>\n</body>\n</html>\n'; candidate=p163(candidate); candidate=inject_canonical_sw(candidate); gates=validate(candidate)
    tmp=MAIN.with_suffix('.tmp'); tmp.write_text(candidate,encoding='utf-8'); tmp.replace(MAIN); print({'status':'NEW_MAIN_GOLD_DIAMOND_READY','target':str(MAIN),'sha256':hashlib.sha256(candidate.encode()).hexdigest(),'bytes':len(candidate.encode()),'gates':gates,'phase_report':phase_report,'cumulative_syntax':cumulative})
