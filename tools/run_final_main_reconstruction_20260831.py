...
def validate_fragments(parts):
    ...
    return [{'part':idx,'bytes':len(part.encode()),'lines':part.count('\n')+1} for idx,part in enumerate(parts,1)]
...
def main():
    ...
    tmp=MAIN.with_suffix('.tmp');tmp.write_text(candidate,encoding='utf-8');tmp.replace(MAIN);print({'status':'NEW_MAIN_GOLD_DIAMOND_READY','target':str(MAIN),'sha256':hashlib.sha256(candidate.encode()).hexdigest(),'bytes':len(candidate.encode()),'gates':gates,'phase_report':phase_report,'full_syntax':full_syntax})
