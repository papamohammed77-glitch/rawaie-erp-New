...
s=surgical_edit(s)
checks={'compat_removed':'/* RAWAEA MAIN2 COMPATIBILITY */' not in s,'auth_once':s.count('/* RAWAEA MAIN2 AUTHORITATIVE MODULE */')==1,'version_once':s.count("window.RW_PWA_RECONSTRUCTION_VERSION='MAIN2-COMPLETE-SURGICAL-v1';")==1,'governed_once':s.count('// MAIN2_GOVERNED_CLOSED:v1')==1,'dash_alias_absent':'window.RW_Dashboard={render:renderDashboard};' not in s,'items_alias_absent':'window.RW_Items={render:renderItems};' not in s,'actions':'var actions=' in s,'delegation':'function main1Delegation(' in s,'main3':'MAIN3' in s,'main11':'MAIN11' in s,'shell':'window.RW_ShellContext' in s,'auth':'window.RW_Auth' in s,'nav':'window.RW_Navigation' in s,'views':'window.RW_Views' in s,'owner_license':'window.RW_OwnerLicense' in s and 'owner_profile' in s and 'license_status' in s,'stock_engine':'post_stock_movement' in s and 'reserve_stock' in s,'finance_reports':all(x in s for x in ('get_trial_balance','get_profit_loss','get_balance_sheet')),'edge_call':'edgeCall' in s,'supabase_client':'supabaseClient' in s or 'createClient' in s,'diamond':'RAWAEA 122 DIAMOND CONTRACT CLOSURE v1' in s,'doctype':s.lstrip().lower().startswith('<!doctype html>')}
p=Parser(); p.feed(s); p.close(); checks.update({'html':p.starts.get('html',0)==1 and p.ends.get('html',0)==1,'body':p.starts.get('body',0)==1 and p.ends.get('body',0)==1,'script_balance':p.starts.get('script',0)==p.ends.get('script',0),'style_balance':p.starts.get('style',0)==p.ends.get('style',0)})
bad=[k for k,v in checks.items() if not v]; print('P163_STATIC_CHECKS',checks)
if bad: fail('static:'+','.join(bad))
TARGET.write_text(s,encoding='utf-8')
print('P163_TARGET_BEFORE_SHA256='+before)
print('P163_TARGET_AFTER_SHA256='+hashlib.sha256(s.encode()).hexdigest())
if len(p.inline)!=1: fail('inline application script count '+str(len(p.inline)))
...
(async()=>{
 ...
 const r=await p.evaluate(()=>{
  ...
  const req=['dashboard','telesales','customers','online-store','pos','orders','runsheets','suppliers','purchase-pos','purchases','receiving','items','branches','picking','loading','delivery','return','unloading','transfer','direct-sale','direct-return','supplier-return','vouchers','vehicle-count','branch-count','general-count','settlement','reports-dashboard','reports-detailed','reports-comprehensive','hr','crm','users','roles','license','settings','audit-log','notifications'];
  const mods=['RW_Dashboard','RW_Items','RW_POS','RW_Orders','RW_Runsheets','RW_Purchases','RW_Warehouse','RW_Finance','RW_Reports','RW_OwnerLicense','RW_HR','RW_CRM','RW_Users','RW_Roles','RW_Views'];
  let denied=false;
  if(S&&S.app&&N&&N.navigate){S.app.currentUser={isOwner:false};try{N.navigate('license')}catch(e){denied=String(e.message||e).includes('OWNER_ONLY')}}
  return {lang:document.documentElement.lang,body:!!document.body,state:!!S,auth:!!window.RW_Auth,nav:!!N,views:!!window.RW_Views,shell:!!window.RW_ShellContext,owner:!!window.RW_OwnerLicense,critical:req.every(v=>flat.includes(v)),mods:mods.every(v=>!!window[v]),denied,count:flat.length,diamond:document.documentElement.outerHTML.includes('RAWAEA 122 DIAMOND CONTRACT CLOSURE v1'),version:window.RW_PWA_RECONSTRUCTION_VERSION||null};
 });
 console.log(JSON.stringify({r,pe,ce}));
 const bad=pe.length||ce.length||r.lang!=='ar'||!r.body||!r.state||!r.auth||!r.nav||!r.views||!r.shell||!r.owner||!r.critical||!r.mods||!r.denied||r.count<30||!r.diamond||r.version!=='MAIN2-COMPLETE-SURGICAL-v1';
 await b.close();
