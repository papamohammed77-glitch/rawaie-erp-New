...
CUR=Path('Current/PWA/main'); ORIG=Path('Original/PWA/main')
TARGET=Path('Current/PWA/main.reconstruction.html'); CTO=Path('Current/CTO')
PARTS=[CUR/f'main{i}.md' for i in range(1,12)]
ORIGINAL=[ORIG/f'main{i}.md' for i in range(1,12)]
...
def protect_js(js):
    ...
    if not ms:return js
    last=ms[-1]; head=js[:last.start()]; tail=js[last.start():]
    head=p.sub(lambda m:'<\\/script'+m.group(0)[m.group(0).lower().find('script')+6:],head)
    return head+tail

def assemble(chunks):
    first=chunks[0]
    if not re.match(r'^\s*<!DOCTYPE html>',first,re.I):raise RuntimeError('MAIN1_IS_NOT_HTML_SHELL')
    blocks=list(re.finditer(r'<script(?P<a>[^>]*)>(?P<b>[\s\S]*?)</script>',first,re.I)); inline=None
    for m in blocks:
...
def validate(c):
    ...
    miss=[x for x in req if x not in c]
