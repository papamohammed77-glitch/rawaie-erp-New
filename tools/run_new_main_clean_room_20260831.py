...
def inline_script_span(html: str):
    ...
    if not body:
        raise RuntimeError('BODY_TAG_MISSING')
    start = html.find('<script', body.end())
    if start < 0:
        raise RuntimeError('INLINE_SCRIPT_NOT_FOUND')
    tag_end = html.find('>', start)
    if tag_end < 0:
        raise RuntimeError('INLINE_SCRIPT_OPEN_INVALID')
    opening = html[start:tag_end + 1]
    if re.search(r'\bsrc\s*=\s*', opening, re.I):
        raise RuntimeError('FIRST_BODY_SCRIPT_IS_EXTERNAL')
    close = html.lower().find('</script>', tag_end + 1)
    if close < 0:
        raise RuntimeError('INLINE_SCRIPT_CLOSURE_NOT_FOUND')
    return start, close + len('</script>'), tag_end + 1, close
...
def validate_target(html: str):
    ...
    if re.search(r"supabase\.from\(['\"]stock_branches['\"]\)\s*\.(?:insert|update|upsert|delete)\s*\(", body, re.I):
        raise RuntimeError('NEW_MAIN_DIRECT_STOCK_WRITER_DETECTED')
...
# P124 trigger marker; Browser E2E intentionally remains paused.
