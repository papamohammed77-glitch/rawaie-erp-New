...
def assemble(chunks: list[str]) -> tuple[str, list]:
    ...
    for m in script_blocks:
        if not re.search(r'\bsrc\s*=', m.group('attrs') or '', re.I):
            inline = m
    if inline is None:
...
def validate(candidate: str) -> dict:
    ...
    if candidate.lower().count('</html>') != 1 or candidate.lower().count('</body>') != 1: raise RuntimeError('DOCUMENT_CLOSURE_INVALID')
    if len(re.findall(r'<script(?![^>]*\bsrc\s*=)[^>]*>', candidate, re.I)) != 1: raise RuntimeError('INLINE_SCRIPT_COUNT_INVALID')
    ...
    for idx,(op,cp) in enumerate(zip(ORIGINAL_PARTS,PARTS),1):
        ...
        os=symbols(op.read_text(encoding='utf-8-sig')); parity[f'main{idx}.md']={k:sorted(set(os[k])-set(cs[k])) for k in os}
    cand=symbols(candidate); losses={k:sorted(v-set(cand[k])) for k,v in current_union.items() if v-set(cand[k])}
...
def main():
    ...
    candidate,repairs=assemble(chunks); parity=validate(candidate)
    TARGET.write_text(candidate,encoding='utf-8'); CTO.mkdir(parents=True,exist_ok=True)
    ...
    EVIDENCE.write_text(json.dumps(evidence,ensure_ascii=False,indent=2),encoding='utf-8'); print(json.dumps(evidence,ensure_ascii=False))
