...
//
// Contract boundaries:
// 1) Company context comes only from RW_ShellContext.
// 2) No app_settings global lookup.
// 3) Item identity follows current DB contract: item_code is globally unique.
// 4) Order/PO reads are company-scoped; child rows are reached by parent id.
// 5) Physical stock is never mutated here. Receiving is delegated to receive-purchase.
// 6) Global exports intentionally remain limited to RW_OnlineStore/RW_Purchases.
// ============================================================
...
var RW_OnlineStore = (function(){
  ...
  function renderCards(){
    ...
    var f=items.filter(function(i){
      ...
      var search=!q||(text(i.name).toLowerCase().indexOf(q)!==-1)||(text(i.item_code).toLowerCase().indexOf(q)!==-1);
  ...
  async function trackOrder(){
    ...
    var code=text(input.value).trim(); if(!code) return;
  ...
  async function openReceive(poCode){
    ...
    try{
      ...
      Swal.fire({title:'استلام بضاعة: '+esc(poCode),html:h,width:'780px',showCancelButton:true,confirmButtonText:'اعتماد الاستلام',confirmButtonColor:'#10b981',cancelButtonText:'إلغاء',preConfirm:function(){
        var received=[];
        items.forEach(function(it,idx){var max=Math.max(0,Number(it.qty_ordered||0)-Number(it.qty_received||0)),q=parseFloat(RW_Main6.id('rec-qty-'+idx)?.value)||0;if(q<0||q>max)throw new Error('كمية الاستلام تتجاوز المتبقي للصنف: '+(it.item_code||it.item_name));if(q>0)received.push({itemCode:it.item_code,itemName:it.item_name,unit:it.unit,receivedQty:q});});
        if(!received.length){Swal.showValidationMessage('أدخل كمية استلام واحدة على الأقل');return false;}return received;
      }}).then(function(r){
