...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: voucher, error: voucherError } = await supabase.from("stock_vouchers").select("id, type, status, from_type, from_id, to_type, to_id").eq("company_id", companyId).eq("voucher_code", voucherCode).maybeSingle()
    ...
    const { data: details, error: detailsError } = await supabase.from("stock_voucher_details").select("item_id, item_code, item_name, qty, received_qty").eq("voucher_id", voucher.id).order("id")
    ...
    if (receivedItems.length === 0) {
      receivedItems = details.map((d) => ({ itemCode: d.item_code, receivedQty: Number(d.qty || 0) - Number(d.received_qty || 0) })).filter((x) => x.receivedQty > 0)
    }
    ...
    const { error } = await supabase.rpc("post_manual_stock_voucher_atomic", { p_company_id: companyId, p_voucher_code: voucherCode, p_operation: "RECEIVE", p_user_email: user.email || "", p_effects: effects })
