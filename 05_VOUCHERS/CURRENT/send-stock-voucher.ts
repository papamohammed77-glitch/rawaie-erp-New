...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: details, error: detailsError } = await supabase.from("stock_voucher_details").select("item_id, item_code, item_name, qty").eq("voucher_id", voucher.id).order("id")
    if (detailsError) throw new Error(detailsError.message)
    if (!details?.length) throw new Error("الإذن لا يحتوي على أصناف")
    const effects = buildSendEffects(voucher, details)
    const { error } = await supabase.rpc("post_manual_stock_voucher_atomic", { p_company_id: companyId, p_voucher_code: voucherCode, p_operation: "SEND", p_user_email: user.email || "", p_effects: effects })
