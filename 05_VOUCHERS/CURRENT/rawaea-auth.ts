...
export async function requireWarehouseVoucherAccess(req: Request) {
  ...
  const { data, error } = await supabase.from("users").select("active_warehouse_role").eq("email", authUser.email || "").maybeSingle()
...
export async function getCompanyContext() {
  const { data, error } = await supabase.from("app_settings").select("company_id, main_branch_id").single()
  if (error || !data?.company_id) throw new Error("سياق الشركة غير محدد بشكل وحيد في الإعدادات")
  return { companyId: data.company_id as string, mainBranchId: data.main_branch_id as string | null }
}
