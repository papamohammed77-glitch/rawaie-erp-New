...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: voucher, error: voucherError } = await supabase.from("stock_vouchers").select("id, type, status").eq("company_id", companyId).eq("voucher_code", voucherCode).maybeSingle()
    ...
    const { error } = await supabase.rpc("complete_manual_stock_voucher_atomic", { p_company_id: companyId, p_voucher_code: voucherCode, p_user_email: user.email || "" })
